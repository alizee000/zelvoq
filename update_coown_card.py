...
if 'MessageCircle' not in content:
    content = content.replace('Users, Coins, Sparkles', 'Users, Coins, Sparkles, MessageCircle, Send')

# Add showChat and chatMessage states
state_addition = """  const [invested, setInvested] = useState(false);
...
content = content.replace('  const [invested, setInvested] = useState(false);', state_addition)

# Replace the action button section
old_action_button = """      {/* Action Button */}
      <div className="mt-5">
        {isFullyFunded ? (
           ...
           >
             {invested ? 'Share Claimed! 🎉' : 'Claim a Share'}
           </button>
        )}
      </div>"""

new_action_button = """      {/* Action Button & Chat */}
